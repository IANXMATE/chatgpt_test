...
\begin{remark}[与 HWAC/ZOT 的边界]
HWAC 的 smooth arc residue compiler 与 ZOT 的 zero-orbit/singular-series
重组属于 G1 前端，不属于 balanced finite-field G2。它们的完整修正版证明见
主审计稿与 GFT toolkit；即使 ZOT 已闭合，也不能修复本稿下面的 Mellin
reciprocal-fibre/DCCS 缺口。
\end{remark}
...
\begin{proof}[式 \textup{(5.4)} 的完整证明]
对 $F,G\in\ell^2(\G\times\G)$，
\begin{align*}
...
\end{align*}
角色正交把内层强制为 $xz=x'z'$。与 $x+z=x'+z'$ 合并，说明
$(x,z)$、$(x',z')$ 是同一多项式 $T^2-RT+xz$ 的有序根，故后者只能是
$(x,z)$ 或 $(z,x)$。当 $x=z$ 时两种选择重合，必须减去一次 diagonal，因而
$\mathcal J^*\mathcal J=I+\mathsf S-\mathsf D$。$\mathcal A$ 与 $\mathcal J$
只差 product 坐标上的 unitary character transform，所以 Gram 相同。
又因 $\mathsf S$ 是自伴酉 involution、$\mathsf D$ 是与之交换的正交投影，
$0\le I+\mathsf S-\mathsf D\le2I$，得到范数界。最后
$\Pi_\Delta h(R,P)=\1_{R=P}h(R,P)$ 显然满足
$\Pi_\Delta^*=\Pi_\Delta^2=\Pi_\Delta$，所以范数为 $1$。
\end{proof}
...
\begin{proof}[式 \textup{(6.1)} 的酉性]
在 counting measure 下，
\[
\|\mathcal U_{\rm fib}f\|_2^2
=\sum_{R\in\G}\sum_{A\in\F_p}|f(R,A/R)|^2
=\sum_{R\in\G}\sum_{B\in\F_p}|f(R,B)|^2,
\]
其中固定 $R$ 后置换 $B=A/R$。逆算子为
$(\mathcal U_{\rm fib}^{-1}f)(R,A)=f(R,AR)$，故它是酉的。
\end{proof}
...
\begin{proposition}[为什么 entrywise shear 不能作为无损接口]
entry multiset 的置换一般不保持 Schatten norm。
\end{proposition}
...
\]
二者 entries 都是两个 $1$ 与两个 $0$，但 singular values 分别为
$(\sqrt2,0)$ 与 $(1,1)$，所以 $\|A\|_{S_4}^4=4$ 而
$\|B\|_{S_4}^4=2$。只有能够写成 $T\mapsto UTV$ 的左右酉作用，或另有
exact intertwining identity 的坐标变换，才可无损运输 Schatten norm。
\end{proof}

\section{古早 GFT compiler 结果为何不能替代 G2 封口}

七份古早记录提供了若干正确而优美的代数结构；重新证明后，它们仍不能填补
\[
\mathrm{DCCS}\longrightarrow\mathrm{KPA8}.
\]
下面把这一逻辑边界写成可检查的定理，而不是依赖时间顺序判断。

\begin{theorem}[exact ModelGlue 不产生 analytic saving]
令 $U,V,W$ 为复向量空间，$\mathfrak M:U\times V\to W$ 双线性，
$P_\sigma\in\operatorname{End}(U\otimes V)$ 且
$\sum_\sigma P_\sigma=I$。若
\[
\mathfrak M_\sigma(u,v)
=\widetilde{\mathfrak M}(P_\sigma(u\otimes v)),
\]
则 $\sum_\sigma\mathfrak M_\sigma(u,v)=\mathfrak M(u,v)$；但是从这些假设
不能推出 $\|\mathfrak M(u,v)\|<\|u\|\|v\|$，也不能推出任何 $p^{-\delta}$ saving。
\end{theorem}

\begin{proof}
第一句由 $\sum P_\sigma=I$ 与线性性立即得到。第二句取
$U=V=W=\mathbb C$、$\mathfrak M(u,v)=uv$、$\Sigma=\{0\}$、$P_0=I$，
并取 $u=v=1$；此时输出范数恰为 $1=\|u\|\|v\|$。把映射乘任意常数还表明
不存在仅由 partition hypotheses 控制的统一小量界。
\end{proof}

\begin{proposition}[EPG 只是 parity carrier 的精确定位]
令 $h=(-1)^\Omega$。任取 divisor gauge $\lambda$，置
\[
...
则对所有 $n\ge1$，
\[
\Lambda(n)=L_\lambda(n)+h(n)D_\lambda(n).             \tag{EPG}
\]
但 (EPG) 本身不给 $D_\lambda$ 的范数界。
\end{proposition}

\begin{proof}
$h$ 完全乘法、$h^2=1$ 且 $\mu h=\mu^2$。将
$\Lambda=\mu*\log$ 逐项乘 $h$，得到
$h\Lambda=\mu^2*(h\log)$；同理
$hL_\lambda=(\lambda h)*h$。相减后再乘 $h$ 即得。最后一句可取
$\lambda=0$；此时等式完全正确，却没有给卷积 $D_0$ 的任何 cancellation。
\end{proof}

\begin{remark}[与 DCCS 的类型不匹配]
ModelGlue 的对象是 principal model 的精确重组；EPG 的对象是整数变量上的
Dirichlet convolution。DCCS (6.2) 要证明的是两个指定有限维 operators 在
basis level 的相等性，并随后运输 Schatten-$8$ norm。前两条定理既没有构造
该 intertwiner，也没有提供 (7.3) 的高矩，因此无法跳过 Step 3--6。
\end{remark}

\begin{center}
\begin{longtable}{p{0.30\textwidth}p{0.18\textwidth}p{0.42\textwidth}}
\toprule
古早对象 & 最终状态 & 与现行 G2 的关系\\
\midrule
\endfirsthead
\toprule 古早对象 & 最终状态 & 与现行 G2 的关系\\\midrule\endhead
Exact Parity--Gauge Decomposition & \PASS
& 精确代数恒等式；只定位 $\vartheta=\pi$ carrier。\\
VDHGT/GFT ModelGlue & \PASS
& exact partition 下主模型守恒；不估计 balanced error。\\
raw BAL-2P 被 BAL-SPARSE 覆盖 & \FAIL
& pure repartition 不创造 roughness；旧 manifest 漏 leaf。\\
LP-TITCH & 已注销
& least-prime anchoring 后不再是 canonical target。\\
$\mathsf{WPD}_\pi$ / old single-jet criterion & \OPEN/已降级
& 条件性 parity route；没有给出当前 reciprocal KPA8。\\
M-PSDC/PBRC 与 ASJT & \OPEN 的旁路
& 属于 endpoint-filter/rough-correlation 路线，不是 G2 已有输入。\\
\bottomrule
\end{longtable}
\end{center}

\section{与 G3 的严格类型边界}

\begin{definition}[ordinary balanced 与 marked-carrier 两个 summand]
令 $\mathscr L$ 是所有 origin-labelled localized leaves 的有限 Hilbert 直和。
按是否携带未被普通 packet coefficient 吸收的 marked prime carrier，定义
\[
\mathscr L=\mathscr L_{\rm bal}\oplus\mathscr L_{\rm pol},
\tag{9.1}
\]
并令 $\Pi_{\rm bal},\Pi_{\rm pol}$ 为两个正交投影。G2 的所有 operator
statements，包括 DCCS 与 KPA8，只在 $\mathscr L_{\rm bal}$ 上陈述；
$\mathscr L_{\rm pol}$ 是 G3 的定义域。
\end{definition}

\begin{proposition}[scope separation]\label{prop:scope-separation}
设 $T$ 是 leaf assembly operator。即使已经证明
\[
\|T\Pi_{\rm bal}\|_{\mathfrak S_8}\le C,
\tag{9.2}
\]
也不能推出 $\|T\|_{\mathfrak S_8}\le C$；还必须单独控制
$T\Pi_{\rm pol}$。反之，G3 中 PCRS、cross-prime incidence 或 P1
trace-pushforward 的证明不产生 DCCS identity (6.2)。
\end{proposition}

\begin{proof}
由正交直和，
$T=T\Pi_{\rm bal}+T\Pi_{\rm pol}$。取 $T\Pi_{\rm bal}=0$ 而
$T\Pi_{\rm pol}$ 为任意大倍数的秩一算子，即得第一句的反例。
第二句是 domain/codomain 的类型事实：PCRS 改写 marked carrier leaf，
而 (6.2) 比较的是 balanced reciprocal coefficient 的两个具体有限维
operators；前者没有给出后者在 basis vectors 上的相等性。
\end{proof}

\begin{remark}
这条边界解释了为什么“G3 中 P2 local backend 很强”不能拿来替代 G2
最后的 DCCS splice；同样，G2 将 ordinary packets 控制住以后，也不能把
prime-block-restricted flatness 静默当作已经证明。主审计稿现已把 G3 中
可闭合的 PCRS、gcd/repeated-prime、double Poisson、cross-prime incidence
及 P1 fixed-modulus/trace lemmas 全部补证，但它们与本稿的 OPEN 项正交。
\end{remark}
...
balanced/polarized scope separation & \PASS & orthogonal direct-sum typing\\
EPG / finite ModelGlue & \PASS & 只给精确重写与主模型守恒，不给小量\\
